...
use ylab::task::{ads1115_task, bmi160_task, btn20_task, led_task, lsm6_multi_task, moi_task};
...
fn init() -> ! {
    ...
    spawn_core1(p.CORE1, unsafe { &mut CORE1_STACK }, move || {
        ...
        executor1.run(|spawner| {
            spawner.spawn(lsm6_task(i2c01, 3, 3, 1)).unwrap();
            spawner.spawn(lsm6_task(i2c11, 3, 3, 1)).unwrap();
